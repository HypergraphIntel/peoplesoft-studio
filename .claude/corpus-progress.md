...
byte changes outside Application Classes.** Baseline is commit `2f04cfc`
(Cycle 17), 23,217/30,209 EXACT, protected 430/430.

### What was implemented

Two of the three Cycle 17 grammar rules that were not yet in the
hand-written wrapper (`encodeApplicationClassProgramV2` in
`src/peoplecode/encoder.ts`):

**Phase 18B — empty-body BODY-GAP.** A member body with zero executable
statements is no longer assumed to need exactly one hardcoded `0x4F`
before it (the old, unconditional behavior). Instead, when
`member.body.trim() === ''`, the wrapper now counts interior blank lines
directly in the raw body text (`Math.max(1, newlineCount - 1)`, the same
counting rule the general encoder already uses elsewhere) and emits that
many `0x4F` markers — zero if the body is perfectly adjacent
(`method NAME\nend-method;`), one or more if there is an interior blank
line. A body containing only comments (non-whitespace, but zero
statements) is UNCHANGED — it still goes through the existing
`encodeMethodBody`/`encodeFragmentInternal` path, which already handles
standalone comments and their own blank-line placement; only the
whitespace-only case is special-cased, per instruction not to generalize
further.

**Phase 18C — inter-member TRANSITION.** The wrapper loop now emits one
`0x4F` per source blank line between one method's `end-method;` and the
next method implementation, in IMPLEMENTATION (source) order — using a
new `transitionBlankLines: number` field on `ApplicationClassMethodMember`
(`src/peoplecode/applicationClassProgram.ts`), computed once during
parsing from the raw source gap between two consecutive implementation
matches. This was previously entirely absent from the hand-written
wrapper for both empty and non-empty bodies. `transitionBlankLines` is
always `0` for the last method in implementation order (no next match to
measure against), so Phase 18D's "no transition after the final member"
rule falls out naturally without an explicit `isLast` check.

**A parser bug found and fixed en route (not itself a Cycle 17 grammar
rule, but required to implement Phase 18B at all):** the original
`implementationPattern` regex's `\s*` immediately after the method name
(and after each `/+ ... +/` comment) is GREEDY and cannot distinguish
ordinary single-line-break indentation from a genuine interior blank
line — it silently absorbed the exact whitespace Phase 18B needs to
inspect, so `member.body` for a case like `editIcon` (definition 29965)
came back as `""` instead of `"\n   \n"`. Fixed by replacing the
single-regex comments/body split with a manual scan
(`parseApplicationClassSource`'s new inline loop) that only skips
leading whitespace when a `/+ ... +/` comment is actually found
immediately after it; once no further comment follows, ALL remaining
text — including any interior blank-line whitespace — becomes `body`
untouched. Verified this does not change `signatureComments` extraction
for the existing working cases (spot-checked `outputSVG`, which still
correctly extracts `"Returns String"`).

### Phase 18E — targeted validation

The six Cycle-16-overlap definitions (29640, 29664, 29965, 29967, 29976,
29977) were re-run individually:

- **29640/29664** (`DesignerAssets()`, no interior blank line): wrapper
  bytes were already correct before this cycle (Cycle 16's own
  investigation had already hex-confirmed `00 2d` matches stored exactly)
  — unaffected by this cycle, as expected (zero blank lines to add).
  First differing offset unchanged at the same position, now confirmed
  to be inside `GetRequireConfigJs`'s `GetHTMLText(HTML.*)` call — an
  unrelated, out-of-scope reference/dependency-numbering issue.
- **29965/29967/29976/29977** (`editIcon()`/`pathIcon()`/`stageIcon()`/
  `stepIcon()`, one interior blank line each): first differing offset
  moved from inside the constructor's own wrapper (offset 147, the
  target of this cycle's fix) to inside the SECOND method's own
  `GetHTMLText(HTML.*)` call (offset ~207-213) — the exact same class of
  unrelated, out-of-scope issue as above. The wrapper/BODY-GAP fix is
  confirmed working: the constructor's own bytes now match stored
  exactly, and the first remaining divergence is provably a different,
  later problem.

### Phase 18F — full validation

- Typecheck: clean.
- Unit suite: 491 tests, 490 pass, 1 pre-existing intentional skip, 0
  failures.
- **Application Class target population** (154 methods-only
  definitions, full-output byte comparison against the Cycle 17
  baseline): 97/154 changed generated bytes (up from 23/154 for Cycle
  16's narrower fix, as expected — TRANSITION affects every multi-method
  class, not only the reference-scope-specific ones). Of the 97: 71
  moved strictly closer to the stored byte length, 26 moved marginally
  farther (spot-checked the largest outlier, definition 28997: its own
  first byte divergence is at offset 122, inside an unrelated `Local
  Rowset` type-descriptor encoding that predates the class-header
  region entirely — its farther total length is a downstream ripple of
  this cycle's byte-count changes elsewhere in the same buffer, not a
  new defect this cycle introduced). 0/154 newly EXACT, 0/154 regressed
  from EXACT (still 1/154, unchanged from Cycle 14 onward).
- **Protected baseline gate**: `npm run corpus:verify` —
  `REGRESSION GATE: PASS`, Improved: 0, Regressed: 0, Unchanged
  failures: 0.
- **Full 30,209-definition corpus**: Exact 23,217 (identical to the
  Cycle 17 baseline), Failed 6,992, classification breakdown unchanged.
- **Generated-bytes comparison against `2f04cfc`**: `git diff --stat --
  src/` touches exactly two files, both exclusively Application-Class
  code (`applicationClassProgram.ts`: parser/IR only;
  `encoder.ts`: only the `encodeApplicationClassProgramV2` wrapper loop
  and the new `transitionBlankLines` consumption, not
  `encodeFragmentInternal`'s own general statement-dispatch logic) — no
  other program kind's generated bytes can be affected, and the
  corpus-wide regression comparison (`Improved: 0, Regressed: 0`)
  confirms no definition's classification changed.

All Phase 18F acceptance criteria are met: zero previously-EXACT
regressions; zero unrelated non-Application-Class byte changes; changes
concentrated in the Application Class population Cycle 17 predicted
(every one of the 97 changed definitions is inside the methods-only
target population).

### Explicitly not done (per instruction)

- No broadening of Application Class syntax support (extends/implements/
  property/instance/constant/abstract/interface remain unsupported,
  unchanged from Cycle 14).
- The non-empty-body `SEPARATOR`→`BODY` single hardcoded `0x4F` was left
  completely unchanged — Cycle 17 did not investigate whether that case
  needs the same blank-line-counting treatment, and generalizing it was
  explicitly out of scope ("do not generalize blank-line handling
  elsewhere").
- The ~10.5% non-clean final-member transition shapes Cycle 17 flagged
  (long byte blobs containing disabled-comment/REM-block encoding) were
  not guessed at; the implementation only emits the proven `0x4F`{1,n}
  shape for non-last transitions and nothing for the last member,
  leaving those shapes exactly as unsupported as before.
- The reference/dependency-numbering issue newly surfaced by Phase 18E's
  own targeted checks (the `GetHTMLText(HTML.*)` divergence common to
  all six focus definitions) was not investigated or patched — it is a
  different axis (Cycle 16's own "full cross-method deduplication...
  explicitly deferred" scope), not a wrapper/BODY-GAP/TRANSITION
  question.

### Next actions

- Next research population (not started): the `HTML.*`/`GetHTMLText`-style
  reference-numbering divergence common to all six former Cycle-16-overlap
  definitions — now the cleanest lead into whatever cross-method
  reference-deduplication behavior Cycle 16 deferred.
- Next research population (carried over from Cycle 17, unchanged): the
  minority "truly-empty" before-terminator shapes and the non-clean
  final-member transition blobs.
...
LOCAL SNAPSHOT throughout.** Baseline is commit `5831dc7` (Cycle 15),
23,217/30,209 EXACT, protected 430/430, 141/154 Application Class
target-scope definitions encoding without throwing, 1/154 byte-exact.

### What was implemented

Cycle 15 proved that the deferred blank-line/declaration-boundary marker
flush decision (`hasCompiledReferences` in `encodeFragmentInternal`) is a
WHOLE-COMPILATION-UNIT property, but Cycle 14 encodes each Application
Class method body as its own, independent `encodeFragmentInternal` call,
so the check only ever saw that one method's own references. This cycle
adds a caller-supplied override so the whole class's answer can be used
instead of each method's local one:

- `EncodeProgramContext.compilationUnitHasCompiledReferences?: boolean`
  (`src/peoplecode/encoder.ts:324`) — when explicitly supplied, overrides
  the local `hasCompiledReferences` computation
  (`src/peoplecode/encoder.ts:10230`) via `??` (not `||`), so an explicit
  `false` is still honored. Every existing caller omits this field, so it
  stays `undefined` and every non-Application-Class call path, and every
  currently-EXACT definition, is provably unaffected.
- A read-only prepass inside `encodeApplicationClassProgramV2`
  (`src/peoplecode/encoder.ts`, immediately after the parsed `methods`
  array becomes available): encodes each method body in ISOLATION via
  `encodeProgramArtifacts`, purely to inspect whether it produces any
  compiled PSPCMNAME reference beyond the blank owner placeholder, and
  reduces that to one boolean, `programHasCompiledReferences`. This
  prepass allocates no real reference, does not affect final reference
  ordering/NAMENUM, and does not change dependency identity,
  DependencyScope, method-local ChainSemantics, or method ordering in the
  real encode below it — it is thrown away immediately after use.
- `programHasCompiledReferences` is threaded into the real
  `encodeFragmentInternal` call inside the `encodeFragment` closure (used
  for both the leading `import` fragment and, via `encodeMethodBody`,
  every method body) as
  `compilationUnitHasCompiledReferences: programHasCompiledReferences`.

No other behavior changed. `suppressDeclarationSectionMarkers` (the
Cycle 14 mechanism gating the two *immediate* `0x2D` push sites for
`closedTopLevelDeclarationSection`/`closedApplicationClassLocalSection`)
is untouched and orthogonal to this fix — it disables a different,
unconditional emission path, not the deferred
`pendingReferenceLocalBoundary`/`pendingReferenceGroupBoundaries` flush
this cycle targets.

### Phase 16D — targeted prediction (performed before full validation)

Reran `tools/corpus/research/appclass-blankline-boundary-analysis.ts`
(unmodified) against the new encoder. The population summary
(target/exact/mismatch/errors, boundary-case counts) was **numerically
identical** to the Cycle 15 report: still 1/154 byte-exact, still 38
boundary cases located, still 26 classified "no-own-references" by that
script's FIRST-DIFF heuristic.

This looked at first like the fix had no effect, so it was checked
directly rather than trusted: a byte-level, non-truncated hash comparison
of every one of the 154 target definitions' full generated output,
before vs. after this cycle's change (`git stash` of `encoder.ts` alone,
re-run, restore), found **23/154 definitions with materially different
generated bytes** — the "first diff offset" the research script reports
did not move for these because, in every one of them, an *earlier*,
unrelated byte divergence (an empty-body constructor method's hand-written
wrapper -- e.g. definition 29640's `method DesignerAssets(); ... end-method;`
with no statements -- places its trailing `0x4F` inter-method separator
adjacently in the wrong position relative to the `0x64 0x15 0x2D` method
terminator) occurs earlier in the byte stream and pins the reported
"first diff" there regardless of what this cycle fixes further along.
That empty-body-wrapper byte-ordering issue is a distinct, unrelated
Cycle-14-era gap, not touched in this cycle, and not one of the 12
residual Cycle 15 marker cases either — it is new information for the
next research population.

Cross-referencing the 26 "no-own-references" boundary cases against
whole-class reference distribution found only 6 (definitions 29640,
29664, 29965, 29967, 29976, 29977) where the whole class has SOME but
not all methods with references (the population this cycle's fix
targets); the other 20 are classes with NO references
anywhere, correctly unaffected controls per Cycle 15's own definition.
Direct inspection confirmed all 6 of those are themselves the same
empty-body-wrapper phenomenon above, not this cycle's target mechanism —
so the *specific* 26-case Cycle 15 population, measured by first-diff
location alone, was already fully explained by other causes, and this
cycle's fix's real, measurable effect lies elsewhere in the same
definitions' later bytes (see below), not at their reported first-diff
offset.

### Phase 16E — full validation

1. **Typecheck**: `npx tsc -p . --noEmit` — clean.
2. **Unit suite**: `npm test` — 491 tests, 490 pass, 1 pre-existing
   intentional skip, 0 failures.
3. **Targeted Application Class controls**: full-output SHA/byte
   comparison across all 154 methods-only target definitions, before vs.
   after, isolating this cycle's change alone:
   - 23/154 changed generated bytes.
   - 0/154 newly EXACT, 0/154 regressed from EXACT (none of the 23 were
     EXACT before or after).
   - Of the 23, 18 moved strictly closer to the stored byte length (by
     1-118 bytes), 4 moved marginally further (2-3 bytes) — consistent
     with Cycle 15's own finding that ~68% of the per-method-scoping
     defect is causal and ~32% is a distinct, opposite-direction
     phenomenon (the same 12-case residual population Cycle 15 already
     flagged and this cycle was explicitly told not to chase); 1 was
     unaffected either way.
4. **Full 154-definition methods-only population sweep**: unchanged
   top-line counts (1/154 byte-exact, same encode-success rate) — expected,
   since none of the 23 changed definitions crossed into full byte
   equality this cycle.
5. **Protected baseline gate**: `npm run corpus:verify` —
   `REGRESSION GATE: PASS`, Improved: 0, Regressed: 0, Unchanged
   failures: 0.
6. **Full 30,209-definition corpus**: Exact 23,217 (identical to the
   Cycle 15 baseline), Failed 6,992, classification breakdown unchanged
   (`EXACT 23217, UNKNOWN_MISMATCH 4541, ENCODE_ERROR 1386,
   DECODE_SOURCE_MISMATCH 814, UNSUPPORTED_SYNTAX 251`).
7. **Generated-bytes/classification comparison against the `5831dc7`
   baseline**: the corpus harness's own regression comparison (step 5)
   already performs this per-definition; combined with the explicit
   per-id hash diff in step 3, every one of the 23 changed definitions is
   confirmed to be inside the Application Class target population, and
   every one of the remaining 30,186 definitions (all non-Application-Class,
   plus the 131 Application Class definitions this cycle did not touch)
   is confirmed byte-for-byte unchanged.

All Phase 16E acceptance criteria are met: zero previously-EXACT
regressions; no non-Application-Class generated-byte changes; reference
ordering/count unchanged (the prepass only ever reads its own throwaway
encode, never the real one); structural-marker changes are concentrated
exactly in the predicted population (Application Class methods-only
definitions whose class-wide reference distribution is "some but not
all"), with the caveat, matching Cycle 15's own advance warning, that
this population turned out to be dominated by a different, coincidental
byte-region overlap (the empty-body-wrapper issue) rather than a clean,
independent confirmation of the 26-case correlation.

### Explicitly not done (per instruction)

- The 12 residual, non-correlating Cycle 15 marker mismatches (including
  the confirmed opposite-direction extra-`0x4F` phenomenon in definition
  29315) were not investigated or patched.
- The newly-surfaced empty-body-constructor-method wrapper byte-ordering
  gap (definitions 29640/29664/29965/29967/29976/29977 and likely others)
  was not patched. It is a distinct, Cycle-14-era hand-written-wrapper
  defect — the unconditional `0x4F` placement immediately after a method's
  signature-comment `0x2D` and before an EMPTY body, versus the real
  compiler's placement of that `0x4F` after the method's `0x64 0x15 0x2D`
  terminator, functioning as the INTER-METHOD separator instead. This is
  next distinct research population, not a marker-scope question.

### Next actions

- Next research population (not started): the empty-body-method wrapper
  byte-ordering gap found incidentally this cycle.
- Next research population (carried over from Cycle 15, unchanged): the
  12 residual non-correlating marker mismatches, including the
  opposite-direction case in definition 29315.
- Datasource mode: LOCAL SNAPSHOT (`tools/corpus/hcdev-snapshot.sqlite`)
  throughout; `--live` was not used.
