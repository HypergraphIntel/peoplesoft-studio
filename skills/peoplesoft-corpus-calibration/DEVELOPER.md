...
The primary goal is not merely syntactically valid PeopleCode. The goal is:
...
Every encoder change must preserve all previously proven corpus behavior.

Do not solve one corpus object by introducing a broader rule unless the broader behavior is supported by evidence.
...
Use `--definition-id` for development targets. It resolves the local SQLite
definition row to the stored seven-part PeopleSoft key before Oracle access.
...
Do not use corpus offset as the durable development target. Offsets are discovery
positions and may change after new PeopleCode objects are inserted, removed, or
reordered.

If the mismatch appears to involve PSPCMNAME allocation, reuse, or a `0x21`
reference operand, enable reference tracing:
...
The local SQLite `definition_id` is the preferred development handle because it
remains stable inside the corpus inventory while the underlying seven-part key
...
This project is effectively building a behavioral specification of PeopleTools' PeopleCode compiler from observed artifacts.

Treat every successful corpus object as a test vector.

Treat every newly proven rule as part of the compiler specification.

Treat every `EXACT` result as protected behavior.

The corpus is not just test data.
